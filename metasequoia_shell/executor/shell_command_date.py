...
from metasequoia_shell.exceptions import ShellCommandExecuteError
...
class CommandExecutorDate(SimuExecutor):
    ...
    def execute(self, simu_process: SimuProcess, command_input: SimuCommandInput) -> SimuCommandOutput:
        ...
        while idx < len(params):
            ...
            if param == "-d":
                ...
                if idx >= len(params):
                    ...
                    return SimuCommandOutput(return_code=0, output_stream=None)  # TODO 待优化
        ...
        return SimuCommandOutput(
            return_code=0,
            output_stream=current.strftime(format_string)
        )
